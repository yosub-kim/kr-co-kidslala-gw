/*
 * $Id: JSONWriter.java,v 1.1 2009/09/19 11:15:41 cvs3 Exp $
 * created by    : jiwoong
...
import kr.co.kmac.pms.project.master.action.ErpProjectAction;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

public class BizplayJSONWriter {
			...
			for (int i = 0; i < props.length; i++) {
				...
				if(!"class".equals(name)){					
					...
					if (value instanceof Date) {
						...
						value = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.sss").format(((Date) value).getTime());
					}
					add(name, value);
					if (i < props.length - 2)
						add(',');
