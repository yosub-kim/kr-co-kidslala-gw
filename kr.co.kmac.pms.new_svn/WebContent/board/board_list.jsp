	...
	<form name="frm" method="post">
		...
		<div class="contents sub">
			...
			<c:if test="${bbsId eq 'qna'}">
				...
				<div class="box">
					<div class="info_box">
						<p class="info_title">회사 업무 시스템  관련 문의 시 신속히 답변 드리겠습니다.</p>
						<p class="info_text">
							· 회사 업무 시스템 활용 방법과 관련된 궁금증 및 활용시 어려운 점이 있을 경우 편하게 문의 주시기 바랍니다.<br />
							· 문의 사항과 관련된 화면을 캡쳐하여 첨부해 주시면 좀 더 정확한 답변이 가능합니다.<br /> 
							· 문의 주신 사항에 대해서는 신속히 확인하여 답변 드리겠습니다.
						</p>
					</div>
					<!-- // info_box -->
				</div>
				...
				<div class="box">
					<div class="link_box">
						<div class="link_ui">
							<p>주요 기준지침 바로보기</p>
							<ul>
								<li><a href="javascript:viewNewbasisbbs('56098');"> <span><i
					...
					<table class="tbl-list all">
						<thead>
							<tr>
								...
								<c:if test="${bbsId eq 'sharebbs' || bbsId eq 'doobitCorps'}"><th>좋아요</th></c:if>
							</tr>
						</thead>
						<tbody>
						<c:forEach var="result" items="${result.list}">
							<tr onmouseout="row_out(this)" <c:choose><c:when test="${result.topArticle == 'Y'}">onmouseover="topArticleRow_over(this);" class="topArticle"</c:when><c:otherwise> onmouseover="row_over(this)"</c:otherwise></c:choose>>
								...
								<td style="cursor:pointer" class="subject" onclick= "location.href='/action/BoardAction.do?mode=boardView&bbsId=<c:out value="${result.bbsId}" />&seq=<c:out value="${result.seq}" />&projectCode=<c:out value="${projectCode}" />&projectName=<c:out value="${projectName}" />'"><p>
								...
								<td class="file">
									<c:if test="${ result.fileCnt > 0 }"><a>첨부</a></c:if>
								</td>
								...
								<c:choose><c:when test="${bbsId eq 'templatebbs' }"></c:when><c:otherwise><td align="center">
									...
									<c:if test="${bbsId eq 'sharebbs' || bbsId eq 'doobitCorps'}"><td align="center"><c:out value="${result.likeCnt }" /></td></c:if>
								</tr>
										</c:forEach>
