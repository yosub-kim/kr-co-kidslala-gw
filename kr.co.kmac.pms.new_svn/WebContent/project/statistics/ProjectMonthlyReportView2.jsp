...
function goDetail(state, projectCode, writerSsn) {
	var year = document.frm.year.value;
	var month = document.frm.month.value;
	if (month < 10) month = "0" + month;
	var params = "&yearMonth="+year+month+"&projectCode="+projectCode+"&writerSsn="+writerSsn;
	switch(state) {
		case "0" :	// 미작성
	...
	var url="/action/ProjectMonthlyReportAction.do?mode=selectListMreportStatDetailList"+params;
	//alert(url);
	window.open(url, 'preportList', 'top=0,left=0,status=no,toolbar=no,scrollbars=yes,width=685,height=550,directories=no,menubar=no');
}
...
<%
	...
	tableGrid.setAttribute(" class='listTable' cellpadding='0' cellspacing='0' width='765' ");
	...
	headerRow.addCell(new Cell("비즈유형").setAttribute(" width='10%' "));
	headerRow.addCell(new Cell("프로젝트명").setAttribute(" width='*' "));
	headerRow.addCell(new Cell("작성").setAttribute(" width='9%' "));		
	headerRow.addCell(new Cell("미작성").setAttribute(" width='9%' "));
	headerRow.addCell(new Cell("검토중").setAttribute(" width='9%' "));
	headerRow.addCell(new Cell("승인중").setAttribute(" width='9%' "));
	headerRow.addCell(new Cell("확정").setAttribute(" width='9%' "));
	...
	if(list == null || list.size() > 0){
		for(ProjectMonthlyReportDataForList monthlyReport : list){
			...
			row.addCell(new Cell(monthlyReport.getProjectName()).setAttribute(" align='left' "));
			/*
			...
			*/
			row.addCell(new Cell("<a href=\"javascript:goDetail('0', '" + monthlyReport.getProjectName() + "\',\'" + monthlyReport.getWriter() + "')\">" + monthlyReport.getIsWrite()+"</a>").setAttribute(" align='center' "));
			row.addCell(new Cell("<a href=\"javascript:goDetail('1', '" + monthlyReport.getProjectName() + "\',\'" + monthlyReport.getWriter() + "')\">" + monthlyReport.getIsNotWrite()+"</a>").setAttribute(" align='center' bgcolor='#FFDFFF' "));
			row.addCell(new Cell("<a href=\"javascript:goDetail('2', '" + monthlyReport.getProjectName() + "\',\'" + monthlyReport.getWriter() + "')\">" + monthlyReport.getIsNotReview()+"</a>").setAttribute(" align='center' "));
			row.addCell(new Cell("<a href=\"javascript:goDetail('3', '" + monthlyReport.getProjectName() + "\',\'" + monthlyReport.getWriter() + "')\">" + monthlyReport.getIsNotApprove()+"</a>").setAttribute(" align='center' "));
			
			row.addCell(new Cell(monthlyReport.getConfirm()).setAttribute(" align='center' bgcolor='#D9ECFF' "));
		...
		/* if (isWriteTotal != 0 || isNotWriteTotal != 0){
			row = new Row();		
			row.addCell(new Cell("합계").setAttribute(" align='center' "));
			row.addCell(new Cell("").setAttribute(" align='center' "));
			row.addCell(new Cell("").setAttribute(" align='center' "));
			row.addCell(new Cell(Integer.toString(isWriteTotal)).setAttribute(" align='center' "));
			row.addCell(new Cell(Integer.toString(isNotWriteTotal)).setAttribute(" align='center' bgcolor='#FFDFFF' "));
			row.addCell(new Cell(Integer.toString(isNotReviewTotal)).setAttribute(" align='center' "));
			row.addCell(new Cell(Integer.toString(isNotApproveTotal)).setAttribute(" align='center' "));
			row.addCell(new Cell(Integer.toString(confirmTotal)).setAttribute(" align='center' bgcolor='#D9ECFF' "));
		...
		row = new Row(); 
		row.addCell(new Cell("검색 결과가 존재하지 않습니다 .").setAttribute(" colspan='8' align='center' "));
		tableGrid.addRow(row); 
	}
		...
		<tr>
			<td height="12">
				<jsp:include page="/common/include/includePageMainTitle.jsp" flush="true">
					<jsp:param name="title" value="프로젝트별 프로젝트 레포트 관리" />
				</jsp:include>
			</td>
		</tr>
			...
			<td height="21" align="left" valign="top">
				...
				<%@ include file="/common/include/searchBox_Header.jsp"%>
					<table height="52" border="0" width="100%" cellpadding="0" cellspacing="0">
						...
						<tr>
							<td class="searchTitle_center">직업구분</td>
							<td class="searchField_center">
								<select name="jobClass"  class="selectbox">
						...
						<tr>
							<td class="searchTitle_center">비즈니스유형</td>
							<td class="searchField_center">
								<SELECT name='businessTypeCode' class='selectbox' style='width: 100pt;'>
							...
							<td class="searchField_center" colspan="3">
								<input type="text" name="projectName" size="30" class="textInput_left" value="<c:out value="${projectName}"/>">
