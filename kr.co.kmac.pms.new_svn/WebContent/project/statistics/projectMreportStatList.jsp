...
<%
	...
	headerRow.addCell(new Cell("컨설턴트명").setAttribute(" width='10%' "));
	headerRow.addCell(new Cell("구분").setAttribute(" width='10%' "));
	headerRow.addCell(new Cell("작성").setAttribute(" width='10%' "));		
	headerRow.addCell(new Cell("미작성").setAttribute(" width='10%' "));
	headerRow.addCell(new Cell("검토중").setAttribute(" width='10%' "));
	headerRow.addCell(new Cell("승인중").setAttribute(" width='10%' "));
	headerRow.addCell(new Cell("확정").setAttribute(" width='10%' "));
	headerRow.addCell(new Cell("Total").setAttribute(" width='10%' "));
	...
	if(list == null || list.size() > 0){
		for(ProjectMonthlyReportDataForList monthlyReport : list){
			...
			*/
			...
			row.addCell(new Cell(monthlyReport.getTotal()).setAttribute(" align='center' "));
			...
			total = total+Integer.parseInt(monthlyReport.getTotal());
		...
		if (isWriteTotal != 0 || isNotWriteTotal != 0){
			...
			row.addCell(new Cell(Integer.toString(total)).setAttribute(" align='center' style='background-color:#F2F1EE' "));
		...
		row = new Row(); 
		row.addCell(new Cell("검색 결과가 없습니다 .").setAttribute(" colspan='9' align='center' "));
		tableGrid.addRow(row); 
	}
		...
		<tr>
			<td height="12">
				<jsp:include page="/common/include/includePageMainTitle.jsp" flush="true">
					<jsp:param name="title" value="프로젝트레포트 월별현황" />
				</jsp:include>
			</td>
		</tr>
