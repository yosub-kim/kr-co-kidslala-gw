...
function doSearch() {
	...
	document.emergencyForm.action = "/action/ProjectEmergencyAction.do?mode=cboData";
	document.emergencyForm.submit();	
}
function goProjectSearchPage(businessTypeCode, projectState, delayState, costOver, endProcess, hasDate)  {
	var params="";
	if(businessTypeCode != '' && businessTypeCode != undefined){
...
function goEmergencySearchPage()
{
	var url="http://intranet.kmac.co.kr/kmac/Education/emergency_list.asp?chk=Y";
	window.open(url, 'emergencyList', 'top=0,left=0,status=no,toolbar=no,scrollbars=yes,width=1585,height=835,directories=no,menubar=no');
}
</script>
</head>
										...
										<c:if test="${!empty list}">
											<c:forEach var="rs" items="${list.list}">
												<tr onmouseover="row_over(this)" onmouseout="row_out(this)">
													<!-- goProjectSearchPage(businessTypeCode, projectState, delayState, costOver, endProcess, hasDate) -->
