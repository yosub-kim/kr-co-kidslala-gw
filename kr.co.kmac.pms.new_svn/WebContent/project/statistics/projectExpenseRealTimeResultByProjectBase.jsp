...
<title>COO 담당 프로젝트에 투입된 모든 인력의 월 전체 성과급</title>
<%-- 공통js,css include --%>
...
function goPage(next_page) {
	...
	document.form.target = "";		
	document.form.action = "/action/RealTimeProjectExpenseAction.do?mode=retrieveExpenseRealTimeResultByProjectBase";
	document.form.submit();
}
function doSearch() {
			...
			<tr>
				<td>
					<jsp:include page="/common/include/includePageMainTitle.jsp" flush="true">
						<jsp:param name="title" value="COO 담당 프로젝트에 투입된 모든 인력의 월 전체 성과급" />
					</jsp:include>
				</td>
			</tr>
			<tr>
				<td height="30">※ 본 내역은 해당 COO 담당 프로젝트에 투입된 인력의 해당 월 예상 성과급 전체를 실시간으로 계산한 것이므로 실 지급된 성과급과 차이가 있을 수 있음.</td>
			</tr>
					...
					<%@ include file="/common/include/searchBox_Header.jsp"%>
						<table border="0" width="100%"  cellpadding="0" cellspacing="0" height="26">
							<tr>
								...
								<td class="searchField_left" width="120">
									<select name="jobClass" class="selectbox" style="width: 99%" onchange="changeDiv(this.value);" <c:if test="${readOnly=='ON'}">disabled</c:if>>
