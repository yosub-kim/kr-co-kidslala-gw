...
function goPage(next_page) {
	...
	document.form.action = "/action/MonthlyReportAction.do?mode=getMonthlyReportStatus";
...
function openWeekluReportList(projectCode, state) {
	var url = "/action/MonthlyReportAction.do?mode=getMonthlyReportList"
		+"&projectCode="+projectCode
		+"&state="+state
		+"&viewMode=monthlyReportStatus"
		+"&readonly=true";
...
function createWreport(projectCode){
	//var ActionURL = "/action/MonthlyReportAction.do?mode=assignMonthlyReportByProjectCode&projectCode="+projectCode;
	var ActionURL = "/action/MonthlyReportAction.do?mode=assignMonthlyReport&projectCode="+projectCode;
...
function createWreportAll(){
	var ActionURL = "/action/MonthlyReportAction.do?mode=assignMonthlyReport";
	var sFrm = document.form;
	var status = AjaxRequest.submit(
		sFrm
		,{ 'url':ActionURL
...
*/
//#######################
function getMonthlyReportDetail(projectCode, assignYear, assignMonth, assignWeekOfMonth) {
		...
		<tr>
			<td width="100%" valign="top">
				<table width="100%" cellpadding="0" cellspacing="0"> 
						...
						<td width="100%">
							...
							<div class="btNbox pb15">
								<div class="btNlefTdiv">				
									...
									<span class="bold colore74c3a"><c:out value="${monthlyReportStatus.valueListInfo.totalNumberOfEntries}"/></span>
									<span>Total - Page(<span><c:out value="${monthlyReportStatus.valueListInfo.pagingPage}"/></span> of <span><c:out value="${monthlyReportStatus.valueListInfo.totalNumberOfPages}"/></span>)</span>
								</div>
					...
					<tr>
						<td>
							<table id="monthlyReportTable" class='listTable' width="100%" cellpadding="0" cellspacing="0">
								<thead id="monthlyReportTableHeader">
									<tr> 
										<td width="7%">상태</td>
										<td width="*">프로젝트명</td> 
										<td width="9%">작성자</td>
								...
								<tbody id="monthlyReportTableBody"> 
									<c:if test="${!empty monthlyReportStatus.list}">
										<c:forEach var="rs" items="${monthlyReportStatus.list}">
									...
									<td width="100%" align="center">
										<SCRIPT LANGUAGE="JavaScript">
											document.write( makePageHtml( 
													<c:out value="${monthlyReportStatus.valueListInfo.pagingPage}" default="1"/>, 
													10, 
													<c:out value="${monthlyReportStatus.valueListInfo.totalNumberOfEntries}" default="0"/> , 
													10)
											) ;
										</SCRIPT>
									</td>
