...
function goPage(next_page) {
	...
	document.form.action = "/action/ProjectExpenseAction.do?mode=projectExpenseHistoryManagement2_coo";
	...
	<div style="margin:70px 15px 0 15px;">
		<div class="board_box">
			<div class="title_both">
				<div class="h1_area">
					<p class="h1"><c:out value="${year}" />년  <c:out value="${month}" />월 본부/센터별 성과급 내역</p>
				</div>
				...
				<table  id="projectProgressTable" class="tbl-edit td-c pd-none"><!-- td 영역이 가운데 정렬 일 경우 td-c -->
					...
					<tbody>
						...
						<c:forEach var="ex" items="${list.list}">
							<tr>
								<c:if test="${ex.ssn !=  ssnChk}">
								<td rowspan="<c:out value="${ex.involvedPrjCnt }" />"><c:out value="${ex.name }"/></td>
								</c:if>
								<c:if test="${ex.ssn !=  ssnChk}">
								<td rowspan="<c:out value="${ex.involvedPrjCnt }" />"><c:out value="${ex.role }"/></td>
								</c:if>
								<c:if test="${ex.ssn !=  ssnChk}">
								<td rowspan="<c:out value="${ex.involvedPrjCnt }" />" style="text-align: right;"><fmt:formatNumber value="${ex.salarySum2 }" type="number"/>&nbsp&nbsp</td>
								</c:if>
