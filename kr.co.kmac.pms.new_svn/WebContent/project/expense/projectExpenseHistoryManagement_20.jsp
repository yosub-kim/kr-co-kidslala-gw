...
<%
	...
	headerRow.addCell(new Cell("담당COO").setAttribute(" align='center' width='14%' "));
	headerRow.addCell(new Cell("프로젝트명").setAttribute(" align='center' width='37%' "));
	headerRow.addCell(new Cell("직업구분").setAttribute(" align='center' width='13%' "));
	headerRow.addCell(new Cell("이름").setAttribute(" align='center' width='16%' "));
	//headerRow.addCell(new Cell("지도일지수").setAttribute(" align='center' width='10%' "));
	headerRow.addCell(new Cell("성과급 합계(원)").setAttribute(" align='center' width='25%' "));
	...
	if(list != null && list.size() > 0){
		for(ExpenseRealTimeResult realTimeResult : list){
			...
			row.addCell(new Cell(realTimeResult.getProjectName()).setAttribute(" align='left' "+(realTimeResult.getIsExceed()!= null && realTimeResult.getIsExceed().equals("Y") ? "background-color: #FFDFFF;" : "")+" ' "));
			row.addCell(new Cell(realTimeResult.getRole()).setAttribute(" align='center' "));
...
<body>
	<form name="form" method="post">
		...
		<table width="765" cellpadding="0" cellspacing="0">
			...
			<tr>
				<td height="12">
					<h3 class="mainTitle"><c:out value="${year}" />년  <c:out value="${month}" />월 프로젝트별 성과급 내역</h3>
				</td>
