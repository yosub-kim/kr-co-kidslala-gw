...
<body> 
<div data-role="page">
	<form id="preportWriteForm" action="">
		...
		<input type="hidden" name="state" id="state" value="<c:out value="${projectReportContent.state}" />" />
		...
		<div data-role="header" data-theme="a" data-position="fixed">
			...
			<a href="/m/web/preport/preportList.jsp" data-icon="arrow-l" data-iconpos="notext" data-direction="reverse">back</a>
			<a href="javascript:executeProjectReport()" data-icon="check" >등록</a>
		</div><!-- /header -->
	
		<div data-role="content">
			<div class="ui-grid-solo" style="text-align: center; ">
				<ul data-role="listview" id="preportListView">
					<li data-role="list-divider">지도일지 작성<c:out value="${projectReportContent.state}" /></li>
				</ul>
			</div>		
			...
			<div data-role="fieldcontain" class="ui-field-contain ui-body ui-br">
				...
				[<c:out value="${projectReportContent.projectName}"/>] <c:out value="${projectReportContent.taskFormTypeName}" />
			</div>
			<div data-role="fieldcontain" class="ui-field-contain ui-body ui-br">
			...
			<div data-role="fieldcontain" class="ui-field-contain ui-body ui-br">
				...
				<textarea name="workContent" id="workContent" placeholder="금번진행사항" style="min-height: 150px;" <c:if test="${readonly}">readonly</c:if>><c:out value="${projectReportContent.workContent}" /></textarea>
			</div>
			<div data-role="fieldcontain" class="ui-field-contain ui-body ui-br">
				<label for="impoInstContent" class="ui-input-text">중요조치사항:</label>
				<textarea name="impoInstContent" id="impoInstContent" placeholder="중요조치사항" style="min-height: 50px;" <c:if test="${readonly}">readonly</c:if>><c:out value="${projectReportContent.impoInstContent}" /></textarea>
			</div>
			<div data-role="fieldcontain" class="ui-field-contain ui-body ui-br">
				<label for="nextPlan" class="ui-input-text">향후일정:</label>
				<textarea name="nextPlan" id="nextPlan" placeholder="향후일정" style="min-height: 50px;" <c:if test="${readonly}">readonly</c:if>><c:out value="${projectReportContent.nextPlan}" /></textarea>
			</div>
			<div data-role="fieldcontain" class="ui-field-contain ui-body ui-br">
				<label for="consultantOpinion" class="ui-input-text">컨설턴트의견:</label>
				<textarea name="consultantOpinion" id="consultantOpinion" placeholder="컨설턴트의견" style="min-height: 50px;" <c:if test="${readonly}">readonly</c:if>><c:out value="${projectReportContent.consultantOpinion}" /></textarea>
			</div>
			<div data-role="fieldcontain" class="ui-field-contain ui-body ui-br">
				<label for="requestContent" class="ui-input-text">지원요구사항:</label>
				<textarea name="requestContent" id="requestContent" placeholder="지원요구사항" style="min-height: 50px;" <c:if test="${readonly}">readonly</c:if>><c:out value="${projectReportContent.requestContent}" /></textarea>
			</div>
			...
			<c:if test="${projectReportContent.state != 'writer'}" >
				<c:if test="${!readonly && (projectReportContent.state != 'writer' && projectReportContent.state != 'reject')}" >
					<div data-role="fieldcontain" class="ui-field-contain ui-body ui-br">
						<label for="consultantOpinion" class="ui-input-text">승인여부:</label>
						<select name="appYN" id="appYN" data-role="slider">
				...
				<div data-role="fieldcontain" class="ui-field-contain ui-body ui-br">
					<label for="revieweOpinion" class="ui-input-text">검토자 의견:</label>
					<textarea name="revieweOpinion" id="revieweOpinion" placeholder="검토자 의견" style="min-height: 50px;" <c:if test="${readonly || projectReportContent.state != 'reviewer'}">readonly</c:if>><c:out value="${projectReportContent.revieweOpinion}" /></textarea>
				</div>
				<div data-role="fieldcontain" class="ui-field-contain ui-body ui-br">
					<label for="approveOpinion" class="ui-input-text">승인자 의견:</label>
					<textarea name="approveOpinion" id="approveOpinion" placeholder="승인자 의견" style="min-height: 50px;" <c:if test="${readonly || projectReportContent.state != 'approver'}">readonly</c:if>><c:out value="${projectReportContent.approveOpinion}" /></textarea>
				</div>
			</c:if>
			<c:if test="${!readonly && (projectReportContent.state == 'approver' || projectReportContent.state == 'reviewer')}" >
				<div data-role="fieldcontain" class="ui-field-contain ui-body ui-br">
					<fieldset data-role="controlgroup" data-type="horizontal" >
						<legend for="payAmount" class="ui-input-text">Rate 조정:</legend>
						<input type="radio" name="payAmount" id="payAmount1" value="0" <c:if test="${projectReportContent.payAmount == '0'}">checked="checked"</c:if>><label for="payAmount1">0</label>
						<input type="radio" name="payAmount" id="payAmount2" value="0.5" <c:if test="${projectReportContent.payAmount == '0.5'}">checked="checked"</c:if>><label for="payAmount2">0.5</label>
						<input type="radio" name="payAmount" id="payAmount3" value="1" <c:if test="${projectReportContent.payAmount == '1'}">checked="checked"</c:if>><label for="payAmount3">1.0</label>
						<input type="radio" name="payAmount" id="payAmount4" value="1.5" <c:if test="${projectReportContent.payAmount == '1.5'}">checked="checked"</c:if>><label for="payAmount4">1.5</label>
						<input type="radio" name="payAmount" id="payAmount5" value="2" <c:if test="${projectReportContent.payAmount == '2'}">checked="checked"</c:if>><label for="payAmount5">2.0</label>
					</fieldset>		
				</div>			
			</c:if>			
