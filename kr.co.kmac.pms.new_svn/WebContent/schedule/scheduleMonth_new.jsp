													...
													<ul>
														...
														<c:if  test="${!empty nDay.dailyProjectLst}">								
														 	<c:forEach var="dailyProjectInfo" items="${nDay.dailyProjectLst}">
																<li class="ico-4"><a href="javascript:;" 
																	onclick="javascript:layer_open(this,'pop_register');"
																		><c:out value="${dailyProjectInfo.projectName}"/></a></li>
															</c:forEach>
														</c:if>
														...
														<c:if  test="${!empty nDay.dailyScheduleLst}"> 
															<c:forEach var="dailyScheduleInfo" items="${nDay.dailyScheduleLst}">
																<c:choose>
																	<c:when test="${dailyScheduleInfo.type eq '휴가' }">
																		<li class="ico-5"><a href="javascript:;" 
																			onclick="javascript:layer_open(this,'pop_register');"
																				><c:out value="${dailyScheduleInfo.content}"/></a></li>
																	</c:when>
																	<c:when test="${dailyScheduleInfo.type eq 'Up-day' }">
																		<li class="ico-3"><a href="javascript:;" 
																			onclick="javascript:layer_open(this,'pop_register');"
																				><c:out value="${dailyScheduleInfo.content}"/></a></li>
																	</c:when>
																	<c:when test="${dailyScheduleInfo.type eq '교육참석' }">
																		<li class="ico-2"><a href="javascript:;" 
																			onclick="javascript:layer_open(this,'pop_register');"
																				><c:out value="${dailyScheduleInfo.content}"/></a></li>
																	</c:when>
																	<c:when test="${dailyScheduleInfo.type eq '사업관리' }">
														...
														<c:if  test="${!empty nDay.customerPickerLst}">
															<li class="ico-1"><a href="javascript:;" 
																onclick="javascript:layer_open(this,'pop_register');"
																	>고객정보<c:out value="${nDay.customerPickerLstCount}"/>건</a></li>
