...
/* 	for(var i=0; i<sFrm.workType.length; i++){
   ...
   /*  <c:if test="${isManager != true }">
	...
	if(sFrm.startHour.value == "10" && sFrm.startMin.value == "30") {
		alert("출근시간은 07:00~10:00 사이만 선택 가능합니다."); return false;
	}
	...
	//}else{
		AjaxRequest.submit(
			...
			,{ 'url':ActionURL
				...
				,'onError':function(obj){alert("이미 근무시간을 입력하였습니다.");}
		});	
	//}
}
...
<table width="700px" cellpadding="0" cellspacing="0">
				...
				<tbody>
					<tr>
						...
						<td class="detailTableTitle_center" width="15%">
							<input type="checkbox" name="multiYN" value="Y" onclick="multiYN_onclick(this);">
							연속입력
						</td>
						<td class="detailTableField_left" width="35%">
							<script>
	...
	<fmt:formatDate var="currTimeMM" value="${currTime}" pattern="MM" />
	<tr>
		<td align="center">
			<div style="overflow: auto;">
				<table class="tableStyle05" id="scheduleDetailTable">
					...
					<%-- 		<td><c:out value="${schedule.startHour}"/>시<c:out value="${schedule.startMin}"/>분 ~ <c:out value="${schedule.endHour}"/>시<c:out value="${schedule.endMin}"/>분</td> --%>
							...
							<td>
								<!-- 현재 월은 삭제 안되도록 수정 -->
								<c:if test="${schedule.month !=  currTimeMM}" >
									<c:if test="${cSsn }" ><a href="#" onclick="deleteSchedule('<c:out value="${sdate}"/>','<c:out value="${schedule.seq}"/>','<c:out value="${ssn}"/>', '<c:out value="${schedule.googleSyncId}"/>');"><img alt="삭제" border="0" src="/images/delete.jpg"></a></c:if>
								</c:if>
							</td>
							<script>
