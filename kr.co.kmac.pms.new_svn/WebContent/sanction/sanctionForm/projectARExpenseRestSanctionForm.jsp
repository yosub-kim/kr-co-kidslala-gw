...
<%
	...
	if(list != null && list.size() > 0){
		...
		row = new Row(); 
		row.addCell(new Cell("데이터가 존재하지 않습니다. ").setAttribute(" align='center' colspan='10' " ));
		tableGrid.addRow(row); 
	}
...
<script type="text/javascript">
	...
	function deleteWork(){
		if(confirm("삭제 하시겠습니까?")) {
			...
			var status = AjaxRequest.submit(
					sFrm,
					{	'url': "/action/ProjectARExpenseRestSanctionAction.do?mode=deleteARExpenseRestSanction",
	...
	function draftRequest(){
		...
		%>
			...
			if($('state').value == "SANCTION_STATE_DRAFT"){
				$('isApproved').value= 'Y';
				ActionURL = "/action/ProjectARExpenseRestSanctionAction.do?mode=createARExpenseRestSanction";
			}else{
				ActionURL = "/action/ProjectARExpenseRestSanctionAction.do?mode=executeARExpenseRestSanction";
	...
	function refTaskRequest(){
		...
		var status = AjaxRequest.submit(
				sFrm,
				{	'url': "/action/ProjectARExpenseRestSanctionAction.do?mode=executeRefWork",
					'onSuccess':function(obj){
						alert('등록하였습니다.');
	...
	function saveToExcel(){
		document.location = '/action/ProjectARExpenseRestSanctionAction.do?mode=saveToExcel&workId='+document.generalSanctionForm.taskId.value;
	}
...
<form id="generalSanctionForm" name="generalSanctionForm" method="post" enctype="multipart/form-data">
	<div id="hiddneValue" style="display: none;">
		...
		<!-- location -->
		<div class="location">
			<p class="menu_title">인센티브 품의하기</p>
			<ul>
								...
								<tbody>
									<tr>
										...
										<td>
											...
											value="<c:choose><c:when test="${sanctionDoc.state == 'SANCTION_STATE_DRAFT'}"><c:out value="${year}"/>년 <c:out value="${month}"/>월 인센티브 품의</c:when><c:otherwise><c:out value="${sanctionDoc.subject}"/></c:otherwise></c:choose>" readonly>
										</td>									
									</tr>
								</tbody>
							</table>
									...
									<tr>
										...
										<c:choose>
											<c:when test="${rs.salaryJobClassDesc == '상근(2)'}">
												<td><fmt:formatNumber value="${rs.salaryTotalRealTimeSalary * (7/10) }" type="number"/></td>
												<td><fmt:formatNumber value="${rs.salaryTotalRealTimeSalary * (3/10) }" type="number"/></td>
											</c:when>
											<c:otherwise>
												<td>-</td>
												<td>-</td>
