						...
						<tbody>
							...
							<tr onclick="detail('<c:out value="${rs.ssn}" />')" style="cursor: hand;">
								<td><c:choose>
									...
									<c:when test="${rs.jobClass eq 'C' }" >엑스퍼트</c:when>
									<c:when test="${rs.jobClass eq 'D' }" >산업계강사</c:when>
									<c:when test="${rs.jobClass eq 'E' }" >대학교수</c:when>
									<c:when test="${rs.jobClass eq 'J' }" >상임</c:when>
									<c:when test="${rs.jobClass eq 'N' }" >RA</c:when>
									<c:when test="${rs.jobClass eq 'R' }" >아르바이트</c:when>
								</c:choose></td>
