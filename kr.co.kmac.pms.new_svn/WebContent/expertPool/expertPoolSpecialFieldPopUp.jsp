...
function specialField_Save(){
	...
	if(specialFieldList.length > 0) {
		var t1 = $(opener).specialFieldDiv.innerHTML; 
		var str = "";
		for(var i = 0; i < specialFieldList.length ; i++) {
			str += "<span id='specialField_'>";
			str += "<input type='hidden' name='specialId' value='"+specialFieldList[i].getAttribute('specialId')+"' />";
			str += "<input type='hidden' name='specialName' value='"+specialFieldList[i].getAttribute('specialName')+"' />";
			str += specialFieldList[i].getAttribute('specialName') + "[" + specialFieldList[i].getAttribute('specialFunctionName') + "]";
			str += "<img src='/images/delete.gif' onclick='$(this).up().remove();' style='cursor: pointer;'>";
			str += "</span> ";
		}
		$(opener).specialFieldDiv.innerHTML = t1 + str;
	}
	window.close();
}
			...
			, 'onSuccess':function(obj){   
	        	...
	        	while($('specialFieldListBody').childElements().size() > 0){
	           	...
	           	specialFieldList.each(function(specialField) {
		        	...
		        	newCellArr[0].innerHTML = "<input type=checkbox id=specialChk specialId="+specialField.specialId+" specialName='"+specialField.specialName+"' specialFunctionName='"+specialField.specialFunctionName+"'>";
	...
	if(specialFieldList.length > 0) {
		for(var i = 0; i < specialFieldList.length ; i++) {
			if(specialFieldList[i].checked) {
	        	...
	        	newCellArr[0].innerHTML = "<input type='checkbox' id='mySpecialChk' specialId='"+specialFieldList[i].getAttribute('specialId')+"' specialName='"+specialFieldList[i].getAttribute('specialName')+"' specialFunctionName='"+specialFieldList[i].getAttribute('specialFunctionName')+"'>";
...
<table width="100%" cellpadding="0" cellspacing="0">
	...
	<tr>
		<td>
			<table width="480" cellpadding="0" cellspacing="0" >
				...
				<!-- 칼럼스타일 정의 종료 -->
				<tr>
					<td><span class="subTitle">상임 컨설턴트 전문분야</span></td>
					<td></td>
					<td><span class="subTitle">나의 전문분야</span></td>
				</tr>
				<tr>
					<td class="detailTableField_center" style="vertical-align: top; padding-top: 4px;">
						...
						<div style="border-color: c7c7c7; border-width: 1px; border-style: none; height: 305px; overflow: auto;">
							<table style="width: 100%" id="specialFieldListTbl">
								...
								<thead>
									<tr>
										...
										<td class="searchTitle_center">전문분야</td>
									</tr>
								</thead>
	...
	<tr>
		<td>
			<table width="480" height='32' bgcolor='#F3F3F3' cellpadding="0" cellspacing="0">
				<tr>
					<td>
						<div class="btNbox txtR">
							<a class="btNe006bc6 txt2btn" href="#" onclick="specialField_Save();">저장</a>
