...
var j$ = jQuery;
	...
	j$(document).ready(function(){
		...
		function createstatusbar0(obj){
			...
			appendStr += "		<select name='attachIsEssential' class='hideme'><option value='0' <c:out value="${preportEssentialFlag}"/> >비필수</option><option value='1'>필수</option></select>";
			...
			appendStr += "	<td <c:if test="${viewMode != 'SIMPLE'}"> style='width: 100px;' </c:if><c:if test="${viewMode == 'SIMPLE'}"> class='hideme' </c:if>><select name='attachOutputName' class='selectbox'><option value='공개' <c:if test="${attachOutputName == '공개'}">selected</c:if>>공개</option><option value='비공개' <c:if test="${attachOutputName=='비공개'}">selected</c:if>>비공개</option></select>";
							...
							<thead id="attachFileTableHeader">
								<c:if test="${fileMode == 'WRITE'}">
									<tr> 
										...
										<td <c:if test="${viewMode != 'SIMPLE'}"> style="width: 130px;" </c:if><c:if test="${viewMode == 'SIMPLE'}"> class="hideme" </c:if>>공개여부</td>
								...
								<c:if test="${fileMode == 'READ'}">
									<c:if test="${!empty fileList0}">
										<tr>
											...
											<td <c:if test="${viewMode != 'SIMPLE'}"> style="width: 140px;" </c:if><c:if test="${viewMode == 'SIMPLE'}"> class="hideme" </c:if>>공개여부</td>
