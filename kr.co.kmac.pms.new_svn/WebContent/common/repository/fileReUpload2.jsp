...
var j$ = jQuery;
	...
	j$(document).ready(function(){
		...
		function createstatusbar0(obj){
			...
			appendStr += "	</td>";
			appendStr += "	<c:if test="${viewMode != 'SIMPLE'}"><td><select name='attachOutputType' class='selectbox'><option></option><c:forEach var="item" items="${attachOutputType}"><option value='<c:out value="${item.key}"/>'><c:out value="${item.detail1}"/></option></c:forEach></select></td></c:if>";
			appendStr += "	<c:if test="${viewMode != 'SIMPLE'}"><td><input type='text' name='attachOutputName' class='textInput_left' style='width: 100%;'/>";
		...
		function createStatusbar(obj){
			...
			appendStr += "		<input name='f_taskId' type='hidden'/>";
			appendStr += "	</td>";
			appendStr += "	<td <c:if test="${viewMode != 'SIMPLE'}"> class='select_area' </c:if><c:if test="${viewMode == 'SIMPLE'}"> class='hideme' </c:if>><select name='attachOutputType' class='selectbox'><option></option><c:forEach var="item" items="${attachOutputType}"><option value='<c:out value="${item.key}"/>'><c:out value="${item.detail1}"/></option></c:forEach></select></td>";
			appendStr += "	<td <c:if test="${viewMode != 'SIMPLE'}"> class='select_area' </c:if><c:if test="${viewMode == 'SIMPLE'}"> class='hideme' </c:if>><input type='hidden' name='attachOutputName' class='textInput_left' style='width: 100%;'/>";
			appendStr += "		<input type='hidden' name='attachFileId'/>";
			appendStr += "		<input type='hidden' name='attachCreatorId' value='<authz:authentication operation="username" />' />";
			appendStr += "		<input type='hidden' name='attachCreateDate'/>";
			appendStr += "		<input type='hidden' name='attachWorkName' />";
...
<table width="100%" cellpadding="0" cellspacing="0">
	<tr>
 		<td><c:out value=""></c:out>
			...
			<br>
			<table id="attachFileTable1" class="tbl-edit td-c">
			<thead id="attachFileTableHeader">
					<c:if test="${fileMode == 'WRITE'}">
						...
						<tr>
							<th class="detailTableField_center">순서</th>
							<th class="detailTableField_center">파일명</th>
							<th class="detailTableField_center">다운로드</th>										
						</tr>
					</c:if>
								...
								<tr>
									...
									<c:choose>
										<c:when test="${isReview== 'TRUE'}">
											<td><input type="text" title="<c:out value="${file.attachFileName}" />" name="attachFileName" id="fileName_<c:out value="${file.attachSeq}"/>" 
									...
									<td class="hideme"
										><input type="hidden" name="attachOutputName" value="<c:out value="${file.attachOutputName}"/>" class="textInput_left" style="width: 100%;"
									...
									<td>
										<%-- <img alt='파일삭제' src='/images/btn_file_delete.png' align='absmiddle' onclick="deleteRowAttachFile(this, '<c:out value="${file.attachFileId}"/>');" class="abort" style="display: inline;"/> --%>
										<img alt="다운로드" src="/images/btn_download.png" width="23px" align="absmiddle" style="cursor: hand;" onclick="fileDownload('<c:out value="${file.attachFileId}"/>')" />
									</td>
								</tr>
							</c:forEach>  
						</c:if> 
